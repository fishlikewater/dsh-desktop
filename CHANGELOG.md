...
- **标题栏状态展示**：拖拽区居中显示「服务在线/连接中/服务离线 · 服务地址」状态文本（与状态圆点同源，服务地址切换即时更新），大幅提升标题栏信息利用率
- **托盘品牌图标**：托盘双态图标从「绿/灰圆点」升级为品牌设计——浅灰渐变圆底 + 黑鲸鱼（与 App 图标同源造型），右下角状态角标区分在线（绿）/离线（灰），带白描边保证深浅任务栏可见

### Fixed

- **窗口缩在左上角（严重）**：窗口状态记忆在隐藏期被采样到极小几何（160x28）并落盘，下次启动窗口缩在角落无法正常使用。修复双保险：保存侧拒绝明显过小的几何（w<400 或 h<300 不落盘）；恢复侧把已存在的过小几何提升到最小窗口尺寸（860x620，与工作区取小），历史坏配置启动即自愈
- **配置弹层文字换行**：弹层宽度 420→520→560px；label 加宽至 104px + nowrap（「服务崩溃自动重启」等长标签完整显示），hint 防换行 + 省略号兜底，所有设置行单行不挤压

### 工程

- 单元测试 80 → 81（窗口状态 clamp 下限提升/零尺寸保持/小屏上限约束 3 例防御测试）
- `.gitignore` 补 `docs/perf.md`、`docs/updater.md`（历史移出跟踪的本地保留文档，消除每次任务 review gate 的误报阻塞）
