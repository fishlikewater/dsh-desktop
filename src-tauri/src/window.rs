...
mod tests {
    ...
    fn clamp_moves_oversized_rect_into_work_area() {
        ...
        // 尺寸低于最小窗口（860x620）时提升到最小尺寸（2026-08-24 事故回归防御）
        let c = clamp_to_work_area((200, 900, 900, 600), (0, 0, 1280, 720));
        assert_eq!(c, (200, 100, 900, 620), "下越界应上移，且尺寸提升到最小高度 620");
    }

    #[test]
    fn clamp_moves_negative_coordinates_into_work_area() {
        // 负坐标（如从第二显示器切回主显示器后 x 为负）→ 归位到工作区原点；
        // 尺寸低于最小窗口（860x620）时提升到最小尺寸（事故回归防御）
        let c = clamp_to_work_area((-400, -200, 800, 600), (0, 0, 1920, 1040));
        assert_eq!(c, (0, 0, 860, 620), "负坐标应归位到工作区原点，尺寸提升到最小窗口");
    }
    ...
    fn clamp_keeps_rect_inside_work_area_unchanged() {
        ...
        assert_eq!(c, (100, 50, 860, 620), "位置保留，尺寸提升到最小窗口");
    }
